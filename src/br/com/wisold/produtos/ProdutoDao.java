...
public class ProdutoDao
{
  ...
  {
    return this.manager.createQuery("select t from Produto t WHERE t.usuario.id = :id").setParameter("id", usuario.getId()).getResultList();
  }
  ...
  {
    return this.manager.createQuery("select t from Produto t WHERE t.usuario.id = :id and t.industria.id = :idIndustria").setParameter("id", usuario.getId()).setParameter("idIndustria", industria.getId()).getResultList();
  }
}
